...
export default function Seguranca({ width, url, title, isCorPrincipal }: iSvg) {
    return (
        <svg width={width ?? 24} height={width ?? 24} className='cor-principal-hover' onClick={() => url && Router.push(url)}>
            <defs>
            ...
            <use xlinkHref='#svg-seguranca' fill='currentColor'></use>
        </svg>
    )
}